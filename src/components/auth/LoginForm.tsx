  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.message?.includes('Invalid login credentials')) {
        setErrorMessage('Email ou senha inválidos. Verifique suas credenciais ou registre-se caso não tenha uma conta.');
      } else {
    ...
    <Card className="w-full max-w-md shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Link
                to="/forgot-password"
                className="text-sm font-medium text-association-primary hover:underline"
              >
                Esqueceu a senha?
              </Link>
            </div>
      ...
      <CardFooter>
        <div className="text-center w-full text-sm">
          Não tem uma conta?{" "}
          <Link to="/register" className="font-medium text-association-primary hover:underline">
            Registre-se
          </Link>
        </div>
      </CardFooter>
